...
export const ResetArea = (props: ResetAreaProps) => {
    ...
    return (
        <div className='m-utility__container'>
            <button className={`m-btn__utility u-txt-white u-textM1 u-font-wb ${resetButtonStyle}`} onClick={reset}>
                <BsArrowCounterclockwise />リセット
            </button>
            <button className={`m-btn__utility u-txt-white u-textM1 u-font-wb ${resetButtonStyle}`} onClick={handleCapture}>
                <BsFillCameraFill /> 共有
            </button>
        </div>
    )
